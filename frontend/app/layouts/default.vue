...
const collapsed = ref(false);      // desktop sidebar collapse
...
<template>
  <div class="min-h-screen bg-slate-100">
    ...
    <div v-if="mobileOpen" class="md:hidden fixed inset-0 bg-black/40 z-40" @click="closeMobile"></div>

    <!-- Mobile slide-out drawer -->
    <aside :class="['md:hidden fixed top-0 left-0 h-full w-64 bg-white z-50 flex flex-col transition-transform duration-200',
      ...
      <div class="px-4 py-5 border-b border-slate-100 flex items-center justify-between">
        ...
        <button @click="closeMobile" class="text-ink-400 hover:text-ink-700 p-1" title="Close">
      ...
      <nav class="flex-1 px-2 py-4 space-y-1">
        <NuxtLink @click="closeMobile" to="/" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-ink-700 hover:bg-slate-100"
          active-class="bg-slate-100 text-ink-900 font-medium">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <span>My Time</span>
        </NuxtLink>
        <NuxtLink v-if="user && user.role === 'admin'" @click="closeMobile" to="/admin-totals"
          class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-ink-700 hover:bg-slate-100"
          active-class="bg-slate-100 text-ink-900 font-medium">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <NuxtLink v-if="isSuperAdmin()" @click="closeMobile" to="/super-totals"
          class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-ink-700 hover:bg-slate-100"
          active-class="bg-slate-100 text-ink-900 font-medium">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <NuxtLink v-if="isSuperAdmin()" @click="closeMobile" to="/admin-users"
          class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-ink-700 hover:bg-slate-100"
          active-class="bg-slate-100 text-ink-900 font-medium">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <div class="px-2 py-4 border-t border-slate-100">
        <button @click="doLogout"
          class="flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm text-ink-500 hover:bg-slate-100 hover:text-ink-900">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div class="flex">
      ...
      <aside :class="['hidden md:flex bg-white border-r border-slate-200 flex-col shrink-0 transition-all duration-200 min-h-screen',
        ...
        <nav class="flex-1 px-2 py-4 space-y-1">
          ...
          <NuxtLink v-if="user && user.role === 'admin'" to="/admin-totals"
            class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-ink-700 hover:bg-slate-100"
            active-class="bg-slate-100 text-ink-900 font-medium" :title="collapsed ? 'Totals' : ''">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 17V7m4 10V11m4 6V9M5 21h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v14a2 2 0 002 2z" />
            </svg>
          ...
          <NuxtLink v-if="isSuperAdmin()" to="/super-totals"
            class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-ink-700 hover:bg-slate-100"
            active-class="bg-slate-100 text-ink-900 font-medium" :title="collapsed ? 'Pay & Rates' : ''">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          ...
          <NuxtLink v-if="isSuperAdmin()" to="/admin-users"
            class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-ink-700 hover:bg-slate-100"
            active-class="bg-slate-100 text-ink-900 font-medium" :title="collapsed ? 'Employees' : ''">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-1.13a4 4 0 10-4-4 4 4 0 004 4z" />
            </svg>
      ...
      <main class="flex-1 overflow-x-auto min-w-0">
        <slot />
      </main>
    </div>
  </div>
